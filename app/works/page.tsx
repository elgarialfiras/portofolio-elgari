...
import Lenis from "lenis"; // PENTING: Dikembalikan untuk smooth scroll
import gsap from "gsap"; // PENTING: Dikembalikan untuk sinkronisasi scroll
...
const allProjects = [
  { 
    ...
    image: "/pb-1.webp", 
    gallery: ["/pb-1.webp", "/pb-3.webp", "/pb-4.webp", "/pb-5.mp4", "/pb-2.webp"],
  ...
  { 
    ...
    image: "/mpk1.webp", 
    gallery: ["/mpk1.webp", "/mpk3.webp", "/mpk4.webp"],
  ...
  { 
    ...
    image: "/sa1.webp", 
    gallery: ["/sa1.webp", "/sa2.webp", "/sa3.webp", "/sa4.webp", "/sa5.webp"],
  ...
  { 
    ...
    image: "/sm7.webp", 
    gallery: ["/sm1.webp", "/sm2.webp", "/sm3.webp", "/sm4.webp", "/sm5.webp", "/sm6.webp", "/sm8.webp"],
  ...
  { 
    ...
    image: "/dkv1.webp",
    gallery: ["/dkv2.mp4", "/dkv3.webp", "/dkv4.webp", "/dkv5.webp"],
  ...
  { 
    ...
    image: "/mi1.webp", 
    gallery: ["/mi1.webp", "/mi2.mp4", "/mi3.webp", "/mi5.webp", "/mi8.webp"],
  ...
  { 
    ...
    image: "/pt1.webp", 
    gallery: ["/pt1.webp", "/pt2.webp", "/pt3.webp", "/pt4.webp", "/pt5.webp", "/pt6.webp", "/pt7.webp", "/pt8.webp"],
  ...
  { 
    ...
    image: "/rh1.webp", 
    gallery: ["/rh1.webp", "/rh2.webp", "/rh3.webp"],
  ...
  { 
    ...
    image: "/tnd1.jpg", 
    gallery: ["/tnd1.jpg", "/tnd2.jpg", "/tnd3.mp4", "/tnd4.jpg"],
...
export default function WorksPage() {
  ...
  // 1. PENGEMBALIAN MESIN SMOOTH SCROLL (Sangat vital untuk mobile)
  useEffect(() => {
    const lenis = new Lenis({ lerp: 0.1, smoothWheel: true, touchMultiplier: 1.5 });
    
    if (isNavOpen || selectedProject) lenis.stop();
    else lenis.start();
  ...
  return (
    <main className="bg-white text-black min-h-screen font-sans selection:bg-black selection:text-white overflow-x-hidden">
      
      {/* --- PRELOADER MASUK --- */}
      <AnimatePresence mode="wait">
        {isEntering && (
          <motion.div key="entrance" variants={slideUp} initial="initial" exit="exit" className="fixed inset-0 z-[2000] flex items-center justify-center bg-[#1C1D20] text-white h-[100dvh]">
      ...
      <AnimatePresence mode="wait">
        {isExiting && (
          <motion.div key="exit" variants={slideUpEnter} initial="initial" animate="enter" className="fixed inset-0 z-[2000] flex items-center justify-center bg-[#1C1D20] text-white w-full h-[100dvh]">
            <h1 className="text-4xl md:text-5xl font-light tracking-tight relative flex items-center justify-center">
              <span className="absolute -left-8 md:-left-10 w-3 h-3 bg-white rounded-full animate-pulse"></span>
      ...
      <AnimatePresence mode="wait">
        {isNavOpen && (
          <motion.div 
            initial={{ x: "100%" }} animate={{ x: "0%" }} exit={{ x: "100%" }} transition={{ duration: 0.8, ease: [0.76, 0, 0.24, 1] }}
            className="fixed top-0 right-0 w-full md:w-[450px] h-[100dvh] bg-[#1C1D20] text-white z-[150] p-12 md:p-20 flex flex-col justify-between shadow-[0_0_50px_rgba(0,0,0,0.5)] overscroll-contain"
          >
            <div className="flex flex-col gap-8 mt-20">
              ...
              <div onClick={(e) => handleNavigate(e, "/")} className="text-5xl font-light cursor-pointer hover:translate-x-4 transition-transform duration-300 w-fit">Home</div>
              
              <div onClick={(e) => { setIsNavOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-5xl font-light cursor-pointer hover:translate-x-4 transition-transform duration-300 flex items-center gap-4 w-fit relative">
                Work 
                <motion.div animate={{ opacity: [1, 0, 1] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} className="absolute -left-8 top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full" />
              </div>
              
              <div onClick={(e) => handleNavigate(e, "/contact")} className="text-5xl font-light cursor-pointer hover:translate-x-4 transition-transform duration-300 w-fit">Contact</div>
            </div>
            
            <div className="flex gap-6 text-sm text-gray-400">
              <a href="https://www.instagram.com/elgarialfiras" target="_blank" className="hover:text-white transition-colors">Instagram</a>
              <a href="https://www.linkedin.com/in/elgarialfiras" target="_blank" className="hover:text-white transition-colors">LinkedIn</a>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="fixed top-8 right-6 md:right-12 z-[200]">
        <MagneticButton>
          <button onClick={() => setIsNavOpen(!isNavOpen)} className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex flex-col items-center justify-center gap-1.5 transition-colors duration-300 touch-manipulation ${isNavOpen ? 'bg-[#3B82F6] text-white' : 'bg-[#1C1D20] text-white hover:bg-gray-800 shadow-xl'}`}>
      ...
      {/* --- GAMBAR MELAYANG --- */}
      <motion.div
        className="fixed top-0 left-0 w-[350px] h-[250px] pointer-events-none z-40 overflow-hidden hidden md:flex items-center justify-center"
      ...
      >
        ...
        <div className="w-full h-full relative transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)]" style={{ transform: `translateY(-${modal.index * 100}%)` }}>
      ...
      {/* --- 2. MODAL POPUP CASE STUDY (Penyempurnaan Mobile Padding & Center) --- */}
      <AnimatePresence>
        {selectedProject && (
          <motion.div 
            className="fixed inset-0 z-[500] flex items-center justify-center p-4 md:p-8 overscroll-contain touch-none"
          ...
          >
            ...
            <motion.div 
              ...
              className="bg-white w-[95vw] md:w-full max-w-5xl max-h-[85dvh] md:max-h-[90vh] rounded-2xl md:rounded-3xl relative z-10 overflow-y-auto overflow-x-hidden p-6 md:p-14 shadow-2xl overscroll-contain"
              initial={{ y: "100%", opacity: 0 }} animate={{ y: "0%", opacity: 1 }} exit={{ y: "100%", opacity: 0 }} transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }}
            >
              <button onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 md:top-10 md:right-10 w-10 h-10 md:w-12 md:h-12 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-200 hover:text-black transition-colors z-[60] touch-manipulation">
                ✕
              </button>

              <div className="flex items-center gap-4 mb-4 md:mb-6 mt-2 md:mt-0">
                <button onClick={() => setSelectedProject(null)} className="text-gray-400 hover:text-black transition-colors font-medium text-xs md:text-sm flex items-center gap-2">← Back</button>
              </div>

              <p className="text-blue-600 font-bold text-[10px] md:text-xs tracking-widest uppercase mb-2">Case Study</p>
              <h2 className="text-4xl md:text-7xl font-bold tracking-tighter mb-4 md:mb-6">{selectedProject.title}</h2>
              <p className="text-lg md:text-2xl text-gray-500 mb-10 md:mb-16 max-w-3xl leading-relaxed">{selectedProject.desc}</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-12 md:mb-16 text-sm md:text-base">
                <div className="flex flex-col gap-6 md:gap-8">
                  <div><h3 className="font-bold mb-2 md:mb-3 text-black">The Problem</h3><p className="text-gray-600 leading-relaxed">{selectedProject.problem}</p></div>
                  <div><h3 className="font-bold mb-2 md:mb-3 text-black">My Role</h3><p className="text-gray-600 leading-relaxed">{selectedProject.role}</p></div>
                </div>
                <div className="flex flex-col gap-6 md:gap-8">
                  <div><h3 className="font-bold mb-2 md:mb-3 text-black">The Output</h3><p className="text-gray-600 leading-relaxed">{selectedProject.output}</p></div>
                  <div><h3 className="font-bold mb-2 md:mb-3 text-black">The Impact</h3><p className="text-gray-600 leading-relaxed">{selectedProject.impact}</p></div>
                </div>
              </div>

              {/* AREA MASONRY GALLERY */}
              <div className="columns-1 md:columns-2 gap-4">
                {selectedProject.gallery?.map((media: string, index: number) => (
                  <div key={index} className="break-inside-avoid mb-4 w-full bg-gray-100 rounded-xl md:rounded-2xl border border-gray-200 overflow-hidden relative">
                    {media.includes(".mp4") ? (
                      <video src={media} autoPlay loop muted playsInline className="w-full h-auto" />
                    ) : (
                      <img src={media} alt={`${selectedProject.title} gallery ${index + 1}`} className="w-full h-auto hover:scale-105 transition-transform duration-500 block" />
                    )}
                  </div>
                ))}
              </div>
      ...
      <section className="pt-10 md:pt-20 pb-20 md:pb-32 px-6 md:px-12 max-w-7xl mx-auto relative z-10">
        <h1 className="text-5xl md:text-6xl lg:text-8xl font-medium tracking-tighter leading-[1.1] md:leading-[0.95]">
          Crafting content, brands,<br className="hidden md:block"/> and creative experiences 
        </h1>
      </section>

      <section className="px-6 md:px-12 max-w-7xl mx-auto pb-40 md:pb-60 relative z-10">
        ...
        <div className="flex flex-col">
          {allProjects.map((project, i) => (
            <div 
              ...
              className="grid grid-cols-12 py-6 md:py-8 border-b border-gray-200 items-center group cursor-pointer hover:bg-gray-50 transition-colors duration-300 md:mx-0 md:px-0"
            >
              <div className="col-span-12 md:col-span-6">
                ...
                <div className="flex justify-between mt-3 md:hidden text-xs text-gray-500">
        ...
        <div className="w-full flex justify-center mt-20 md:mt-32">
          <MagneticButton>
            <a href="https://drive.google.com/drive/folders/17FlrQiLxyJZ5JfHkNzTgS2LW_Y92Jjt3" target="_blank" rel="noopener noreferrer" className="px-8 py-4 md:px-10 md:py-5 rounded-full border border-gray-300 hover:bg-black hover:text-white transition-colors duration-300 text-sm md:text-base font-medium flex items-center gap-3 group">
              View Full Archive
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:rotate-45 transition-transform duration-300">
      ...
      {/* --- FOOTER --- */}
      <section className="relative w-full bg-[#1C1D20] text-white pt-32 md:pt-40 pb-20 px-6 md:px-12 z-20">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] md:w-[120%] h-[100px] md:h-[250px] bg-[#1C1D20] rounded-[100%] z-[-1]"></div>
        <div className="max-w-7xl mx-auto flex flex-col relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 md:gap-12 border-b border-gray-700 pb-12 md:pb-16 relative">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-4 md:gap-6">
                ...
                <h2 className="text-5xl md:text-[7rem] font-light tracking-tighter">Let's work</h2>
              </div>
              <h2 className="text-5xl md:text-[7rem] font-light tracking-tighter">together</h2>
            </div>
            ...
            <div onClick={(e) => handleNavigate(e, "/contact")}>
              <MagneticButton>
                <div className="w-28 h-28 md:w-48 md:h-48 bg-[#3B82F6] rounded-full flex items-center justify-center cursor-pointer hover:bg-blue-600 transition-colors duration-300 shadow-2xl">
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-8 md:mt-12">
            <a href="mailto:hello@elgarialfiras.com" className="px-6 py-4 md:px-8 md:py-5 rounded-full border border-gray-700 hover:bg-white hover:text-black transition-colors duration-300 text-sm font-medium w-fit">hello@elgarialfiras.com</a>
            <a href="#" className="px-6 py-4 md:px-8 md:py-5 rounded-full border border-gray-700 hover:bg-white hover:text-black transition-colors duration-300 text-sm font-medium w-fit">+62 821 1465 5830</a>
          </div>
        </div>
      </section>
    </main>
